...
def init(
    ...
    console.print(f"[green]✓[/] Configurado! Documentos serão salvos em: [bold]{out_path}[/]")
...
def document(
    ...
    if not diff.strip():
        console.print("[yellow]⚠ Nenhuma alteração detectada. Faça alterações no código e tente novamente.[/]")
        raise typer.Exit()
    ...
    console.print(f"\n[green]✓[/] Documento gerado: [bold]{filepath}[/]")
